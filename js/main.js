...
function getReply(question) {
    ...
    for (const [category, topics] of Object.entries(faqDatabase)) {
        const categoryAliases = topics.alias ? topics.alias.map(a => a.toLowerCase()) : [];
        const categoryKeywords = [category.toLowerCase(), ...categoryAliases];
        for (const keyword of categoryKeywords) {
            if (question.includes(keyword)) {
                for (const [topic, answers] of Object.entries(topics)) {
                    if (topic === 'alias' || topic === 'general') continue;
    ...
    const defaultReplies = [
        "您可以向我询问关于剪纸、皮影、风筝、脸谱、泥塑、竹编等非遗文化的相关知识。例如：'剪纸有哪些技法'、'皮影戏是怎么表演的'等。",
        "请问您想了解哪方面的非遗知识呢？可以问我关于剪纸、皮影、风筝、脸谱、泥塑、竹编的具体问题。",
        "非遗文化博大精深！我可以为您介绍剪纸艺术、皮影戏、风筝、戏曲脸谱、泥塑和竹编工艺等方面的知识。",
        "我可以为您解答剪纸、皮影、风筝、脸谱、泥塑、竹编等方面的知识，请问您具体想了解哪一项？"
    ];
